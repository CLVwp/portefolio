      ...
      >
        {parts.map((part, j) => {
          if (part.startsWith("`") && part.endsWith("`")) {
            return (
              <code
                className="bg-white/10 px-1.5 py-0.5 font-mono text-[0.9em] text-white/90"
...
export default function BlogPostPage() {
  ...
  return (
    <div className="relative min-h-[100dvh] overflow-x-clip bg-black text-white">
      ...
      <main className="mx-auto w-full max-w-3xl px-4 py-24 md:px-8 md:py-32">
        <Link
          className="inline-flex items-center gap-2 font-mono text-xs tracking-[0.2em] text-white/50 uppercase transition-colors hover:text-white"
          href="/blog"
        >
          {t.blog.back}
        </Link>
        ...
        <div className="mt-12 flex flex-wrap items-center gap-3">
          <span className="border border-white/10 px-3 py-1 font-mono text-[10px] tracking-[0.15em] text-white/60 uppercase">
        ...
        <div className="mt-4 h-px w-full bg-gradient-to-r from-violet-500/50 to-transparent" />
        ...
        <div className="mt-24 border-t border-white/10 pt-12">
          ...
          <div className="mt-6 border-t border-l border-white/10">
            {others.map((other) => (
              <Link
                className="group flex items-center justify-between gap-4 border-r border-b border-white/10 p-5 transition-colors duration-500 hover:bg-white/5"
                href={`/blog/${other.slug}`}
                key={other.slug}
              >
                <span className="font-medium tracking-tight text-white/80 transition-colors group-hover:text-white">
                  {other.title[lang]}
                </span>
